...
static void	add_new_heap_free_blocks(t_heap_group *heap_pos, t_heap_group *new_heap, size_t alloc_size)
{
	...
	if (tmp_free)
	{
		tmp_free->size += alloc_size;
		tmp_free->aligned_size += aligned_alloc_size;
	}
	else
	{
		heap_pos->free_block->size = alloc_size;
		heap_pos->free_block->aligned_size = aligned_alloc_size;
	}

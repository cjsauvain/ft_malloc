...
LINUX_RUN=./run.sh
# test source code
TEST0_SRC=tests/test0.c
TEST1_SRC=tests/test1.c
TEST2_SRC=tests/test2.c
TEST3_SRC=tests/test3.c
TEST4_SRC=tests/test4.c
TEST5_SRC=tests/test5.c

compite_test_with_lib() {
    FILE_SRC=${1}
    FILE_EXEC=${2}
    echo -e "${CYAN}Compile ${FILE_SRC} ${RESET}"
    gcc -o ${FILE_EXEC} ${FILE_SRC} -L. -lft_malloc -g
}
...
compile_real_test_get_minor() {
    ...
    gcc -o ${FILE_EXEC} ${FILE_SRC} -g && /usr/bin/time -v ./${FILE_EXEC} 2> .tmp/out_${FILE_EXEC}
